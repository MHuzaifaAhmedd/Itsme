...
import { SharafulQuranCustomNode } from './SharafulQuranCustomNode';
...
interface SharafulQuranCaseStudyDiagramProps {
  diagramId: string;
...
const nodeTypes: NodeTypes = {
  custom: SharafulQuranCustomNode,
};
...
function getLayoutConfig(diagramId: string): {
  ...
  switch (diagramId) {
    case 'system-architecture':
      return { direction: 'LR', nodeWidth: 200, nodeHeight: 100, rankSep: 280, nodeSep: 80 };
    case 'authentication-flow':
      return { direction: 'TB', nodeWidth: 180, nodeHeight: 80, rankSep: 100, nodeSep: 60 };
    case 'core-feature-lifecycle-booking':
      return { direction: 'TB', nodeWidth: 180, nodeHeight: 70, rankSep: 90, nodeSep: 50 };
    case 'core-feature-lifecycle-course':
      return { direction: 'TB', nodeWidth: 180, nodeHeight: 70, rankSep: 90, nodeSep: 50 };
    case 'data-processing-flow':
      return { direction: 'TB', nodeWidth: 180, nodeHeight: 80, rankSep: 100, nodeSep: 60 };
    case 'performance-decision-flow':
      return { direction: 'TB', nodeWidth: 180, nodeHeight: 70, rankSep: 85, nodeSep: 50 };
    default:
      return { direction: 'TB', nodeWidth: 180, nodeHeight: 80, rankSep: 100, nodeSep: 70 };
  }
}
...
export default function SharafulQuranCaseStudyDiagram({
  diagramId,
  nodes: initialNodes,
  edges: initialEdges,
  title,
  height = '650px',
}: SharafulQuranCaseStudyDiagramProps) {
  ...
  const reactFlowEdges: Edge[] = useMemo(() => {
    return initialEdges.map((edge, index) => ({
      ...
      labelStyle: { fontSize: '10px', fill: '#a3a3a3', fontWeight: '500' },
      labelBgStyle: { fill: '#171717', fillOpacity: 0.95 },
      labelBgPadding: [6, 4] as [number, number],
      labelBgBorderRadius: 4,
      markerEnd: { type: MarkerType.ArrowClosed, width: 16, height: 16, color: '#525252' },
    }));
  }, [initialEdges]);
  ...
  const uniqueTypes = useMemo(() => {
    ...
    initialNodes.forEach(n => {
      if (n.type && nodeTypeColors[n.type]) types.add(n.type);
    });
    ...
    <div className="w-full rounded-xl overflow-hidden border border-neutral-800 bg-neutral-950">
      ...
      <div style={{ height }} className="bg-neutral-950">
        ...
        >
          ...
          />
          <MiniMap
            nodeColor={node => nodeTypeColors[(node.data?.type as string) || ''] || '#525252'}
