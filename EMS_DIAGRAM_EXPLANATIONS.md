# EMS Case Study: Diagram Explanations

## How to Use These Diagrams

Each diagram in this document represents a different aspect of the Employee Management System architecture. The diagrams are designed to be rendered using React Flow, D3.js, or similar graph visualization libraries using the node and edge data provided in `ems.case-study.diagrams.json`.

**Visual Legend:**
- **Blue nodes:** Frontend applications and client-side components
- **Green nodes:** Backend services and business logic
- **Orange nodes:** Data storage (MongoDB, Redis, job queues)
- **Purple nodes:** External services and integrations
- **Edges (arrows):** Data flow direction with labels describing the data or action
...
**Defense in Depth:** Instead of a single authentication check, requests pass through 8+ validation layers. This provides security redundancy—if one layer fails (e.g., CSRF tokens disabled for mobile app), other layers (JWT + device validation) still protect the system.
...
## Diagram 3: Attendance Check-In/Out Lifecycle
...
**Main Synchronous Path (Left Side, ~80-170ms total):**
...
## Diagram 4: Task Assignment & Verification Lifecycle

### What It Represents

This diagram shows the complete multi-actor workflow of task management, involving three participants (Admin, Backend, Employee) across four distinct phases:
1. **Task Creation:** Admin creates task with assignment type (individual/department/global)
2. **Notification:** Assignees notified via WebSocket + FCM
3. **Employee Completion:** Employee works on task, uploads proof files to S3, marks complete
4. **Admin Verification:** Admin reviews proof files, approves or rejects completion

The diagram emphasizes the **bidirectional communication** (admin ↔ backend ↔ employee) and **state transitions** of the Task record (pending → active → completed → pending_verification → verified).

### How to Read the Flow

**Phase 1: Task Creation (Top Section)**
...
12. Backend updates `completedBy` array entry: `status: 'completed', proofFiles: [S3 URLs], verificationStatus: 'pending_verification'`
...
16. Backend updates `verificationStatus: 'verified' | 'rejected'`
...
**Array-Based Completion Tracking:** The diagram shows the Task model uses a `completedBy` array with one entry per assignee. This design enables:
- **Independent tracking:** Each employee's completion tracked separately (status, proofFiles, verificationStatus)
- **Partial completion visibility:** Admins see which employees completed vs. pending
- **Re-submission support:** If rejected, employee can re-upload proof without creating new task

**Why S3 for Proof Files:** Proof files flow directly from employee to S3 (via presigned URL), then S3 URLs stored in MongoDB. This design avoids:
- Large file uploads through backend (saves bandwidth, reduces latency)
- File storage in MongoDB (documents limited to 16MB, file storage not optimal use case)
- Complicated file serving logic (S3 handles range requests, CDN distribution, durability)

**Performance Integration (Right Side):** Notice the diagram shows task completion triggering performance calculation cache invalidation. This illustrates how the performance system is **event-driven**—every task completion potentially changes the Completion (C) and Timeliness (T) metrics, requiring leaderboard recalculation on next request.

---

## Diagram 5: Performance Calculation Flow (Four-Metric System)
...
**Cache Hit Path (Fast Path, Left Side):**
...
3. **For each employee (loop):**
   - Aggregate attendance data: `(presentDays, scheduledDays, totalLateMinutes)`
   - Aggregate task data: `(assignedTasks, completedTasks, onTimeTasks)`
   - Calculate A metric: `(presentDays / scheduledDays) * 100`
   - Calculate P metric: `max(0, 100 - (totalLateMinutes / scheduledDays))`
   - Calculate C metric: `(completedTasks / assignedTasks) * 100`
   - Calculate T metric: `(onTimeTasks / completedTasks) * 100`
   - Apply **strict_zero policy:** If any metric missing data, use 0% (not exclude)
   - Fetch configurable weights from SystemSettings (cached in Redis)
   - Calculate final score: `(A*30 + P*20 + C*30 + T*20) / 100` (example weights)
   - Clamp score to 0-100%
   - Assign performance band: Outstanding(≥95%), Excellent(≥85%), Good(≥70%), Needs Improvement(≥50%), Unsatisfactory(<50%)
...
**strict_zero Policy (Center of Diagram):** This is the most critical engineering decision in the performance system. The diagram shows a dedicated node for "Apply strict_zero Policy" after individual metric calculations. This policy states:

- **If attendance data missing (new employee):** A = 0%, P = 0% (not 100% or excluded)
- **If task data missing (no tasks assigned):** C = 0%, T = 0% (not 100% or excluded)
- **If employee deletes tasks:** C still calculated on original assigned tasks (task soft-delete only)

**Why This Matters:** Without strict_zero, employees could game the system:
- New employee with no data would score 100% (unfair advantage)
- Employee could delete assigned tasks to inflate completion rate
- Missing metrics would be excluded, rescaling weights (inconsistent scoring)

**Cache TTL Trade-off:** The diagram shows Redis cache with 3-minute TTL. This means:
- **Pro:** Leaderboard updates every 3 minutes (near real-time for practical purposes)
- **Pro:** Reduces MongoDB query load by ~95% (caching 20+ aggregation queries per employee)
- **Con:** Leaderboard may show slightly stale data for up to 3 minutes after attendance/task changes
- **Mitigation:** Cache invalidated explicitly when attendance or task records saved (post-save hooks)

**Serial vs. Parallel Calculation (Loop Node):** The diagram shows a loop ("For Each Employee") indicating **serial calculation** (one employee at a time). This is a known performance bottleneck (100 employees = 100 sequential aggregations). However, it's acceptable because:
1. Leaderboard cached for 3 minutes (calculation runs infrequently)
2. Individual aggregations fast (~2-5ms each with proper indexes)
3. Future optimization: parallel calculation with `Promise.all()` (noted in limitations)

**Configurable Weights (Bottom Section):** The diagram shows weights fetched from Redis-cached SystemSettings, highlighting the system's flexibility. Different organizations can prioritize different metrics (e.g., increase Punctuality weight to 30% for time-sensitive operations) without code changes.
...
- **Layout:** Layered (hierarchical) or force-directed
- **Direction:** Left-to-right or top-to-bottom
- **Groups:** Use visual grouping/clusters for frontend, backend, data, external layers
- **Node Sizing:** Scale node size based on importance (e.g., Express API larger than individual services)

**Authentication Flow:**
- **Layout:** Vertical sequential flow (top-to-bottom)
- **Emphasis:** Highlight middleware chain as linear sequence
- **Branching:** Show decision points where requests may be rejected (401/403 responses)

**Attendance Lifecycle:**
- **Layout:** Mixed (main path left-to-right, parallel paths branching vertically)
- **Emphasis:** Clearly separate synchronous (solid lines) vs. asynchronous (dashed lines) paths
- **Timing:** Annotate edges with latency estimates

**Task Lifecycle:**
- **Layout:** Multi-actor swimlanes (Admin lane, Backend lane, Employee lane)
- **Phases:** Use horizontal sections for creation, notification, work, verification
- **State Transitions:** Highlight state changes in Task model (status, verificationStatus)

**Performance Calculation:**
- **Layout:** Decision tree with cache hit/miss branches
- **Loop:** Use subgraph or visual container for "For Each Employee" loop
- **Formula Nodes:** Display formulas prominently for A, P, C, T calculations

### Interactive Features to Add

1. **Hover Details:** Show full tech stack, line counts, TTL values on node hover
2. **Edge Animation:** Animate data flow along edges (especially for real-time WebSocket paths)
3. **Click to Filter:** Click node type (e.g., "frontend") to highlight all frontend nodes
4. **Zoom/Pan:** Large diagrams need zoom controls for detail inspection
5. **Time Slider:** For Attendance/Task diagrams, show temporal progression with slider
6. **Metric Breakdown:** For Performance diagram, click employee node to show A/P/C/T breakdown

---

**These diagrams are data-driven from the actual EMS codebase analysis. All nodes, edges, and architectural decisions are verified from PROJECT_ANALYSIS.md and caseStudyData.json. No assumptions or fictional enhancements were added.**
